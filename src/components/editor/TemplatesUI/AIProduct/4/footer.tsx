...
export function AIProduct4Footer({ props, theme }: BlockComponentProps<FooterProps>) {
  ...
  return (
    <footer 
      ...
      style={{ backgroundColor: bg, color: ink, borderColor: `${accent}25` }}
    >
      <div 
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16 pb-16 border-b" style={{ borderColor: `${accent}20` }}>
          ...
          <div className="lg:col-span-2 pr-6">
            <div className="flex items-center gap-3 mb-6">
              {props.logo ? (
                ...
                className="h-9 w-9 rounded-xl flex items-center justify-center text-white shadow-lg"
                style={{ backgroundColor: accent, boxShadow: `0 0 20px ${accent}60` }}
              >
                <Cpu className="h-5 w-5 text-white" />
              </div>}
              <Editable value="O. CHEN // AI ARCHITECTURE" onChange={() => {}} className="font-mono text-sm font-bold tracking-widest uppercase" />
            </div>
            
            <p className="text-xs opacity-65 leading-relaxed max-w-sm mb-8">
              Production-grade AI systems, vector search pipelines, and agentic state machines built for enterprise speed, security, and scale.
            </p>

            <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border text-[11px] font-mono" style={{ backgroundColor: `${accent}10`, borderColor: `${accent}25` }}>
              <Activity className="h-3.5 w-3.5 text-emerald-400 animate-pulse" />
              <span>SYSTEMS_OPERATIONAL // 99.99% UPTIME</span>
            </div>
          </div>

          <div>
            <div className="text-xs font-mono font-bold uppercase tracking-widest mb-6" style={{ color: accent }}>Capabilities</div>
            <ul className="space-y-3.5 text-xs opacity-65 font-mono">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 font-mono text-xs opacity-60">
          <div>
          ...
          <button
            onClick={scrollToTop}
            className="flex items-center gap-2 hover:opacity-100 transition-opacity p-2.5 rounded-xl border"
