...
export function AIProduct4About({ theme }: BlockComponentProps<any>) {
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {[
              ...
              { icon: Database, title: "Hybrid Vector RAG", desc: "Combining dense vector embeddings with sparse BM25 keyword search to eliminate hallucination in document retrieval.", color: "#38BDF8" },
              { icon: Lock, title: "Private LLM Isolation", desc: "Self-hosted fine-tuned open-source models inside VPC boundaries with SOC2-compliant data handling.", color: "#34D399" },
              { icon: FastForward, title: "Quantized Inference", desc: "Optimized vLLM deployments providing sub-200ms time-to-first-token under heavy concurrent traffic.", color: "#FBBF24" }
            ].map((card, i) => (
              <div 
                key={i} 
                ...
                style={{ 
                  ...
                  boxShadow: `0 10px 30px ${bg}80` 
                }}
              >
                <card.icon className="h-8 w-8 mb-6 transition-transform group-hover:scale-110 duration-300" style={{ color: card.color }} />
                <Editable as="h3" value={card.title} onChange={() => {}} className="text-lg font-bold mb-3" />
                <Editable as="p" value={card.desc} onChange={() => {}} className="text-xs leading-relaxed opacity-65" />
              </div>
            ))}
          </div>

          <div className="relative rounded-3xl overflow-hidden border min-h-[400px] group" style={{ borderColor: `${accent}30`, backgroundColor: bg }}>
            <img
            ...
            <div className="absolute bottom-8 left-8 right-8 p-6 rounded-2xl border backdrop-blur-xl" style={{ backgroundColor: `${bg}EE`, borderColor: `${accent}35` }}>
              <div className="font-mono text-xs mb-1 font-bold" style={{ color: accent }}>PROPRIETARY RAG PIPELINE</div>
              <div className="text-sm font-bold">Real-Time Contextual Ingestion Loop</div>
            </div>
          </div>
